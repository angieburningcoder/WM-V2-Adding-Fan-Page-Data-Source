...
- 以粉專為主體的單一 Case List（不再拆出「可忽略清單」，一律用 Case Status 分類）
- Source badges、關聯廣告數、粉專風險與廣告風險兩種分數、處理狀態
- 案件詳情 Drawer：粉專資訊（含截圖眼睛）→ 關聯廣告清單（每則小眼睛）→ 風險判斷 → 原始偵測紀錄 → 案件紀錄
...
- 搜尋支援多關鍵字（空白／逗號／頓號分隔，任一命中即顯示），比對粉專名稱、粉專編號、Case ID、廣告編號與廣告文案
